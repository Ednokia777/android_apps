...
public class Level9 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        previewimg.setImageResource(R.drawable.ukaz_o_volnih_xlebopashcah);
        ...
        numLeft = random.nextInt(16);//генерируем случайное число от 0 до 16.
        img_left.setImageResource(array.images9[numLeft]);//достаем из массива картинку
        text_left.setText(array.texts9[numLeft]);//достаем из массива текст

        numRight = random.nextInt(16);//генерируем случайное число от 0 до 16.
        ...
        text_right.setText(array.texts9[numRight]);//достаем из массива текст

        numDownLeft = random.nextInt(16);//генерируем случайное число от 0 до 16.
        while(numLeft == numDownLeft || numRight == numDownLeft){
            numDownLeft = random.nextInt(16);
            if(numLeft == numDownLeft || numRight == numDownLeft){
                continue;
            }
        }
        img_down_left.setImageResource(array.images9[numDownLeft]);//достаем из массива картинку
        text_down_left.setText(array.texts9[numDownLeft]);//достаем из массива текст

        numDownRight = random.nextInt(16);//генерируем случайное число от 0 до 16.
        while(numLeft == numDownRight || numRight == numDownRight || numDownLeft == numDownRight){
            numDownRight = random.nextInt(16);
            if(numLeft == numDownRight || numRight == numDownRight || numDownLeft == numDownRight){
                continue;
            }
        }
        img_down_right.setImageResource(array.images9[numDownRight]);//достаем из массива картинку
        text_down_right.setText(array.texts9[numDownRight]);//достаем из массива текст
